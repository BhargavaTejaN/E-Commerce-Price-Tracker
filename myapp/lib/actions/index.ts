...
export async function getProductById(productId: string) {
  try {
    ...
    if (!product) {
      console.log(`No Product Found With This ProductId ${productId}`);
      throw new Error(`No Product Found With This ProductId ${productId}`);
    } else {
      return product;
    }
  } catch (error: any) {
...
export async function getSimilarProducts(productId: string) {
  try {
    ...
    // Use the $sample aggregation stage to fetch random products
...
export async function addUserEmailToProduct(
  ...
  try {
    ...
    console.log("Error While adding Email To Product : ", error);
